...
**webserv** — это учебная реализация высокопроизводительного веб-сервера, написанная строго по стандарту **C++98** в рамках проекта School 42.
...
5. [Разбор модулей проекта](#разбор-модулей-проекта)
6. [Flow программы (симуляция работы)](#flow-программы)
7. [Итоги и выводы](#итоги)
...
Простыми словами: это «официант» интернета. Клиент говорит «принеси мне страницу», сервер находит нужный контент (файл, результат скрипта, данные из базы) и отдаёт его обратно.
...
**Ваш webserv** по архитектуре ближе всего к **Nginx** — event-driven модель с неблокирующими сокетами.
...
Именно эту архитектуру вам предстоит реализовать.
...
В проекте вы работаете напрямую с **Berkeley sockets**:
...
**HTTP (HyperText Transfer Protocol)** — stateless, текстовый протокол прикладного уровня.
...
В рамках проекта School 42 полноценная реализация HTTPS **обычно не требуется**, но понимание концепции обязательно.
...
### Какую функцию мультиплексирования использовала команда?

**Наша команда использовала функцию `poll()`**.
...
- **Высокая портативность** — работает одинаково хорошо на Linux, macOS и других Unix-системах, что важно для проекта School 42.
...
### Как работает `select()` и его эквивалент `poll()`

**I/O Multiplexing** (мультиплексирование ввода-вывода) — это ключевой механизм, позволяющий одному потоку эффективно обслуживать **множество** клиентских соединений одновременно без создания отдельного процесса или потока на каждого клиента.

#### Почему multiplexing необходим?
...
Это приводит к крайне низкой масштабируемости. Multiplexing решает проблему: ядро ОС сообщает программе, на каких дескрипторах можно выполнять операции чтения или записи.
Как работает классический select()
Cint select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout);
Принцип работы:

Программа заполняет битовые маски (fd_set), указывая, за какими дескрипторами нужно наблюдать на чтение, запись или исключения.
Вызывается select().
Ядро блокирует выполнение до появления событий или истечения таймаута.
После возврата программа проверяет, какие биты остались в масках.

Недостатки select():

Ограничение на количество дескрипторов (~1024).
Неэффективное копирование масок при каждом вызове.
Линейная сложность обработки (нужно сканировать все дескрипторы).
Неудобная работа с динамически меняющимся набором соединений.

Как работает poll() (используется в webserv)
poll() — более современный и удобный механизм, который мы и применили.
...
Чёткое разделение ответственности: Server занимается только multiplexing и диспетчеризацией, класс Connection — логикой HTTP-запроса и CGI.
...
Что делает: Выводит в поток ошибок (std::cerr) инструкцию по правильному запуску сервера, если юзер ввел фигню (например, запустил ./webserv файл1 файл2 файл3).
...
2. Конструкция try { ... } catch в main
Зачем это нужно: Твой парсер конфигурации (ConfigParser) и движок сервера активно используют исключения (throw std::runtime_error(...)). Если в файле конфигурации забыта точка с запятой или порт уже занят другой программой, сервер не упадет молча с Segmentfault. Исключение «вылетит» в самый верх — в main(), поймается в catch (const std::exception &e), красиво напечатает в консоль Fatal: <описание ошибки> красным цветом и чисто завершит программу с кодом 1.

Что делает catch (...): Это «улавливатель абсолютного форс-мажора». Если где-то вылетит что-то, что не наследуется от std::exception (например, кто-то напишет throw 42;), сервер всё равно перехватит этот выпад, напишет unknown error и не сдохнет грязно.

3. Разбор развилок argc (Логика запуска)
У тебя есть три железных сценария работы, зашитых в main:
...
ЧАСТЬ 1: Файлы данных — Config.hpp и Config.cpp
